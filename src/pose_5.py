...
def minimize_marginals(graph, initial_estimate, pose_options):
    ...
    marginals = marginals = gtsam.Marginals(graph, result)
...
def minimize_errors(graph, initial_estimate, pose_options):
    ...
    err_x1 = marginals.marginalCovariance(X(1)).sum()
    err_x2 = marginals.marginalCovariance(X(2)).sum()
    err_x3 = marginals.marginalCovariance(X(3)).sum()

    # TODO: compute the sum of the errors and return it along with the best pose and landmark
    sum_of_errors = err_x1 + err_x2 + err_x3
